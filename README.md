...
# 清两表 + FAISS
python -m scripts.clear_experiences --include-simulation-records --yes
...
│   ├── scoring.py            # scene_to_vector / payload 映射 / validate
...
进入容器内的 MySQL 交互终端（执行后按提示输入密码 `rootpw`）：

```bash
docker exec -it multi_agent_uav_system mysql -uroot -p
```

非交互式直接带密码（密码 `rootpw`）：

```bash
docker exec -it multi_agent_uav_system mysql -uroot -prootpw
```
...
-- 查看仿真记录表结构
DESCRIBE simulation_records;

-- 查最近 5 条记录
SELECT id, task_id, device_id, simulation_time FROM simulation_records ORDER BY id DESC LIMIT 5;
```
...
**Score 公式：** 已废弃，经验库不计算 / 不存储 score 字段。检索直接按 FAISS L2 距离 ASC 排序。
...
- 类型：`IndexIDMap2(IndexFlatL2)`，23 维
...
| 端点 | 方法 | 描述 |
|---|---|---|
| `/experience/add` | POST | 新增一条经验（MySQL + FAISS 双写） |
| `/experience/{id}` | GET | 获取单条经验完整记录 |
| `/experience/{id}` | DELETE | 删除（FAISS + MySQL 同步） |
| `/experience/search` | POST | 按 scene 检索 Top-K 经验（按 L2 距离 ASC） |
...
2. 构造 23 维 `scene_vector`（`scene_to_vector`）
3. **MySQL 先提交**（拿 `experience_id`）
4. FAISS 写入原始向量
5. **FAISS 失败 → MySQL 补偿删除**（双写一致性）

### 读取流程

1. 当前 scene → 23 维 vector
2. FAISS L2 距离 Top-K（k 由 config 控制，上限 `topk_max`）
3. MySQL `SELECT WHERE id IN (...)` 取完整记录
4. 按 **L2 距离 ASC** 排序返回（最相似优先）
...
Agent 在做参数决策时能直接看到"类似场景下历史经验里用过什么参数 + 当时跑出来的 pdr / delay"，无需修改任何工具。
